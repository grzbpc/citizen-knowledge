---
layout: post.njk
title: Thousands lose water pressure. Two celebrities keep green lawns. Nobody checks either.
date: 2026-08-19
order: 1
source: "Metro, Wednesday 19 August 2026"
part: I
partTitle: When the Law Stops Meaning What It Says
chapter: 2
chapterTitle: Selective Enforcement and Threshold Logic
subheading: When Rules Become Instruments
description: "South East Water has been told disruption to supplies is 'simply not good enough' as thousands face shortages again, while Jeremy Clarkson and Rod Stewart are accused of keeping their lawns green through the same hosepipe restrictions with no comparable scrutiny."
book_reference: "This entry sets an unenforced restriction on two visible landowners against an enforced one on thousands of ordinary customers, illustrating the manuscript's argument that a rule applied selectively does not reduce the underlying behaviour, it simply relocates who is seen breaking it."
has_article_quote: true
---

South East Water has told customers that levels in its drinking water storage tanks remain "critically low" and asked people to use water only for essential purposes, even as supplies were restored to 7,400 homes in Herne Bay after outages. In Benenden, Kent, residents spelled out "HELP US" in a dry field. Wessex Water imposed a fresh hosepipe ban yesterday, just as much of England experienced its first real rain in a month. The Department for Environment, Food and Rural Affairs said SEW "needs to go further and faster" to reduce leaks and follow its drought plan.

On the same page, Jeremy Clarkson and Rod Stewart were named among those accused by some observers of ignoring hosepipe restrictions, after aerial photographs appeared to show well-watered lawns at both men's properties. Clarkson's response was that a borehole, not a hosepipe, was responsible. Neither claim has been formally investigated. Neither man faces a fine.

<blockquote class="quote--manuscript"><p>A system that enforces rules unevenly does not reduce wrongdoing; it redistributes it.</p><span class="cite">The Performance of Obedience, Ch. 2</span></blockquote>

The redistribution here is visible on a single page. A water company is publicly rebuked by name for supply failures affecting thousands of households in Kent. Two named individuals are accused, in the same edition, of the more direct offence of using a hosepipe during a ban that carries a criminal penalty, and the matter ends with a joke about a borehole. Nobody is checking the borehole claim. Nobody needs to. A hosepipe ban depends on the same thing every discretionary rule depends on: someone watching, and someone deciding it is worth the bother to look.

South East Water's failure is structural and gets a government department's name attached to the criticism. The alleged individual breaches are personal, unproven, and closed within the same article by a quip. Both sit under headlines about water shortage on the same day. Only one carries any operational consequence.
